...
public class QarEngDomain {
	...
	private String engfilename;
	private String time;
	private int ias;
	private String cityFromR;
	private String cityToR;
	private String dateR;
	private float egt1;
	private float egt2;
	private float eprEng1Actua;
	private float eprEng2Actua;
	private float eprEng1;
	private float eprEng2;
	private float ff1;
	private float ff2;
	...
	private int raltc;
	private int altStdc;
	private float fqtyTon;
	private float tat;
	private int gw;
	private float headTrue;
	private float drift;
	private float roll;
	private int vref_1;
	private float pitch_1;
	private float fltPathAngl;
	private float winSpd;
	private float winDir;
	private String apuOn;
	private int v2;
	private int v1;
	private int vr;
	private int vref_2;
	private float pitch_2;
	private int acType;
	private float machBuff;
	private float pitchTrm;
	private float pitchRate;
	private float headLin;
	private int flapc;
	private float ctlClFcAL;
	private float ctlClFcBL;
	private float glideDevc;
	private float locDevc;
	private String ldgSeldw;
	private int apEgd1;
	private int apEgd2;
	private String apOff;
	private String atsEgd;
	private int ivv;
	private float tla1;
	private float tla2;
	private String gpwsMode;
	private String masCau;
	private int iasc;
	private int gsc;
	private int egt1c;
	private int egt2c;
	private float oip1;
	private float oip2;
	private float ailQadPosL;
	private float oilPrs1;
	private float oilPrs2;
	private float rawOip1A;
	private float rawOip1B;
	private float rawOip2A;
	private float rawOip2B;
	private String ldgc;
	private String ldgl;
	private String ldgnos;
	private String ldgr;
	private float n11;
	private float n12;
	private float vibN11;
	private float vibN12;
	private float n21;
	private float n22;
	private float vibN21;
	private float vibN22;
	private float n31;
	private float n32;
	private float vibN31;
	private float vibN32;
	private float oilQty1;
	private float oilQty2;
	private float oilTmp1;
	private float oilTmp2;
	private float satr;
	private int dfc;
	private String altStdIsis = "";
	...
	public String toSql() {
		...
		return id + ",'" + tailnr + "','" + fltdt + "','" + engfilename
				+ "','" + time + "'," + ias + ",'" + cityFromR + "','"
				+ cityToR + "','" + dateR + "'," + egt1 + "," + egt2 + ","
				+ eprEng1Actua + "," + eprEng2Actua + "," + eprEng1 + ","
				+ eprEng2 + "," + ff1 + "," + ff2 + ",'" + flightPhase + "','"
				+ flightNo1 + "','" + flightNo2 + "'," + gs + "," + head + ","
				+ tas + "," + mach + "," + vrtg + "," + latg + "," + longit
				+ "," + lonpc + "," + latpc + "," + raltc + "," + altStdc + ","
				+ fqtyTon + "," + tat + "," + gw + "," + headTrue + "," + drift
				+ "," + roll + "," + vref_1 + "," + pitch_1 + "," + fltPathAngl
				+ "," + winSpd + "," + winDir + ",'" + apuOn + "'," + v2 + ","
				+ v1 + "," + vr + "," + vref_2 + "," + pitch_2 + "," + acType
				+ "," + machBuff + "," + pitchTrm + "," + pitchRate + ","
				+ headLin + "," + flapc + "," + ctlClFcAL + "," + ctlClFcBL
				+ "," + glideDevc + "," + locDevc + ",'" + ldgSeldw + "',"
				+ apEgd1 + "," + apEgd2 + ",'" + apOff + "','" + atsEgd + "',"
				+ ivv + "," + tla1 + "," + tla2 + ",'" + gpwsMode + "','"
				+ masCau + "'," + iasc + "," + gsc + "," + egt1c + "," + egt2c
				+ "," + oip1 + "," + oip2 + "," + ailQadPosL + "," + oilPrs1
				+ "," + oilPrs2 + "," + rawOip1A + "," + rawOip1B + ","
				+ rawOip2A + "," + rawOip2B + ",'" + ldgc + "','" + ldgl
				+ "','" + ldgnos + "','" + ldgr + "'," + n11 + "," + n12 + ","
				+ vibN11 + "," + vibN12 + "," + n21 + "," + n22 + "," + vibN21
				+ "," + vibN22 + "," + n31 + "," + n32 + "," + vibN31 + ","
				+ vibN32 + "," + oilQty1 + "," + oilQty2 + "," + oilTmp1 + ","
				+ oilTmp2 + "," + satr + "," + dfc + ",'" + altStdIsis + "'";
	}
}
